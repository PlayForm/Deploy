 ...
 * See the License for the specific language governing permissions and
 * limitations under the License.
 */class Ne{constructor(e){this.container=e}getPlatformInfoString(){return this.container.getProviders().map(n=>{if(Re(n)){const s=n.getImmediate();return`${s.library}/${s.version}`}else return null}).filter(n=>n).join(" ")}}function Re(t){const e=t.getComponent();return e?.type==="VERSION"}const O="@firebase/app",F="0.8.4";/**
 * @license
 * Copyright 2019 Google LLC
 *
 * Licensed under the Apache License, Version 2.0 (the "License");
 ...
 * See the License for the specific language governing permissions and
 * limitations under the License.
 */const dt="firebase-heartbeat-database",ft=1,g="firebase-heartbeat-store";let C=null;function Y(){return C||(C=Te(dt,ft,{upgrade:(t,e)=>{switch(e){case 0:t.createObjectStore(g)}}}).catch(t=>{throw u.create("idb-open",{originalErrorMessage:t.message})})),C}async function ut(t){var e;try{return(await Y()).transaction(g).objectStore(g).get(X(t))}catch(n){if(n instanceof b)p.warn(n.message);else{const s=u.create("idb-get",{originalErrorMessage:(e=n)===null||e===void 0?void 0:e.message});p.warn(s.message)}}}async function j(t,e){var n;try{const r=(await Y()).transaction(g,"readwrite");return await r.objectStore(g).put(e,X(t)),r.done}catch(s){if(s instanceof b)p.warn(s.message);else{const r=u.create("idb-set",{originalErrorMessage:(n=s)===null||n===void 0?void 0:n.message});p.warn(r.message)}}}function X(t){return`${t.name}!${t.options.appId}`}/**
 * @license
 * Copyright 2021 Google LLC
 *
 * Licensed under the Apache License, Version 2.0 (the "License");
 ...
 * See the License for the specific language governing permissions and
 * limitations under the License.
 */function _t(t){$(new I("platform-logger",e=>new Ne(e),"PRIVATE")),$(new I("heartbeat",e=>new gt(e),"PRIVATE")),y(O,F,t),y(O,F,"esm2017"),y("fire-js","")}_t("");var yt="firebase",It="9.14.0";/**
 * @license
